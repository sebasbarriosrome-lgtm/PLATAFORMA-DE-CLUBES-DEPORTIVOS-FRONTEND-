    ...
    <div className="min-h-screen bg-slate-950 text-slate-100">
      <header className="border-b border-white/10 bg-slate-900/80 backdrop-blur sticky top-0 z-40">
        <div className="mx-auto flex w-full max-w-7xl items-center justify-between gap-3 px-4 py-3 sm:px-6">
          <Link to="/" className="flex items-center gap-2 text-lg font-black text-emerald-300">
            <div className="h-9 w-9 rounded-full bg-emerald-400 text-slate-950 flex items-center justify-center font-black">CZ</div>
            ClubZone
          </Link>
          <nav className="flex flex-wrap items-center gap-2 text-sm text-slate-200">
            <Link className="hover:text-white" to="/clubs">Explorar clubes</Link>
            <Link className="hover:text-white" to="/crear-club" state={{ from: "clubs" }}>Crear mi club</Link>
            <Link className="hover:text-white" to="/register" state={{ from: "clubs" }}>Registrarse</Link>
            <Link className="hover:text-white" to="/login" state={{ from: "clubs" }}>Iniciar sesión</Link>
          </nav>
        </div>
      </header>
      ...
      <main className="mx-auto w-full max-w-7xl space-y-6 px-4 py-8 sm:px-6">
        <section className="rounded-3xl border border-white/10 bg-slate-900/70 p-6 shadow-lg shadow-black/30">
          <h1 className="text-4xl font-extrabold text-white sm:text-5xl">Directorio de clubes</h1>
          <p className="mt-2 text-slate-300">Descubre clubes deportivos, explora sus categorías y solicita tu inscripción.</p>
        ...
        {clubsFiltered.length === 0 ? (
          ...
          <section className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {clubsFiltered.map((club) => (
              <article key={club.id} className="group overflow-hidden rounded-2xl border border-white/10 bg-slate-900/70 shadow-lg shadow-black/40 transition hover:-translate-y-0.5 hover:border-emerald-400/30">
                ...
                <div className="p-5">
                  <div className="mb-3 flex items-center justify-between gap-3">
                    ...
                    <span className={`rounded-full px-3 py-1 text-xs font-semibold ${stateClasses[club.state]}`}>
                      {club.state}
                    </span>
                  </div>
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    <span className="rounded-full bg-white/10 px-3 py-1 text-xs text-slate-200">{club.city}</span>
                    <span className="rounded-full bg-white/10 px-3 py-1 text-xs text-slate-200">{club.categories.join(" • ")}</span>
                    <span className="rounded-full bg-white/10 px-3 py-1 text-xs text-slate-200">{club.staff} entrenadores</span>
                  </div>
